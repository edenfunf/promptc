"""Render the promptc analysis as terminal output or JSON.

This module contains every code path that turns analysis results into user-
facing output. The CLI in `cli.py` parses arguments and orchestrates the
analyzer; everything that produces bytes for a human (Rich panels, tables,
JSON) lives here.

Public entry points:
    make_console()         build a Console safe for legacy Windows code pages
    print_terminal(...)    render the full Rich terminal report
    print_json(...)        write the JSON-format report to stdout
"""
...
from promptc.dedup import DedupResult, DuplicateGroup
from promptc.exposure import EXPOSURE_NARRATIVE, ExposureReport
